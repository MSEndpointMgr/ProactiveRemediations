<#
.SYNOPSIS
    Analyses a VC++ Redistributable dependencies CSV and produces a prioritised vendor/app impact report.

.DESCRIPTION
    This script processes the CSV output from Parse-VCRedistReport.ps1 and produces
    actionable conclusions for legacy VC++ Redistributable remediation planning:
...
.PARAMETER CsvPath
    Path to the VC++ Redist dependencies CSV file (output from Parse-VCRedistReport.ps1).
...
.EXAMPLE
    .\Analyze-VCRedistReport.ps1 -CsvPath "C:\Users\user\Downloads\VCRedistApps.csv"

.EXAMPLE
    .\Analyze-VCRedistReport.ps1 -CsvPath ".\VCRedistApps.csv" -Threshold 100 -Top 30

.EXAMPLE
    .\Analyze-VCRedistReport.ps1 -CsvPath ".\VCRedistApps.csv" -OutputPath ".\Analysis.csv"

.NOTES
    FileName: Analyze-VCRedistReport.ps1
    Author: Anders Ahl
    Created: 2026-06-02
    Updated: 2026-06-02
    Version: 1.0.0
    Requires: PowerShell 5.1 or higher
    Use Case: Post-processing analysis of Parse-VCRedistReport.ps1 output
...
[CmdletBinding()]
param(
    [Parameter(Mandatory = $true, HelpMessage = "Path to the VC++ Redist dependencies CSV file.")]
...
Begin {
    ...
    function Get-VCVersionFromMethod {
        param([string]$Method)

        # Extract VC++ version from detection method string
        if ($Method -match 'VC\+\+ (\d{4}) \((\d+\.\d+)\)') {
            return "VC++ $($Matches[1]) ($($Matches[2]))"
        }
        if ($Method -match '(8\.0|9\.0|10\.0|11\.0|12\.0)') {
            $verMap = @{
                '8.0'  = 'VC++ 2005 (8.0)'
                '9.0'  = 'VC++ 2008 (9.0)'
                '10.0' = 'VC++ 2010 (10.0)'
            ...
            return $verMap[$Matches[1]]
        }
        if ($Method -match '2005') { return 'VC++ 2005 (8.0)' }
        if ($Method -match '2008') { return 'VC++ 2008 (9.0)' }
        if ($Method -match '2010') { return 'VC++ 2010 (10.0)' }
        if ($Method -match '2012') { return 'VC++ 2012 (11.0)' }
        if ($Method -match '2013') { return 'VC++ 2013 (12.0)' }
...
Process {
    try {
        ...
        # === VC++ Version Distribution ===
        Write-Host "=== VC++ Version Distribution ===" -ForegroundColor Cyan
        Write-Host ""

        $versionCounts = @{}
        foreach ($app in $csv) {
            $vcVer = Get-VCVersionFromMethod -Method $app.DetectionMethod
            if (-not $versionCounts.ContainsKey($vcVer)) {
                $versionCounts[$vcVer] = 0
            }
            $versionCounts[$vcVer]++
        }
        $versionCounts.GetEnumerator() | Sort-Object Value -Descending | ForEach-Object {
            Write-Host "  $($_.Key): $($_.Value) apps"
        }
        Write-Host ""
        ...
        $vendorSummary | Format-Table -Property @(
            @{ Label = 'Vendor'; Expression = { $_.Vendor }; Width = 25 }
        ...
        Write-Host "  Legacy VC++ versions referenced: $($versionCounts.Count)"
        Write-Host ""
