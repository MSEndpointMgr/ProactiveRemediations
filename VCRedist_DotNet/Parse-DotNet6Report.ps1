<#
.SYNOPSIS
    Parses Intune Remediation CSV export and extracts all detected .NET 6.x dependent applications.

.DESCRIPTION
    This script processes the "DeviceRunStatesByProactiveRemediation" CSV export from
    Microsoft Intune and extracts the detection output from MWPW_PRD_DotNet6Detect.
    It parses the pipe-delimited detection strings and produces a consolidated list
    of all applications found to depend on .NET 6.x across all reported devices.
...
.PARAMETER OutputPath
    Path for the output CSV file. Defaults to the same directory as the input
    with a timestamped filename.

.PARAMETER PostFilter
    Exclude known .NET SDK/runtime tooling paths (dotnet\sdk, dotnet\shared) from results.
    Useful when parsing reports collected before the detection script added these exclusions.

.PARAMETER Detailed
    Include per-device breakdown in console output.

.EXAMPLE
    .\Parse-DotNet6Report.ps1 -CsvPath "C:\Users\user\Downloads\DeviceRunStatesByProactiveRemediation.csv"

.EXAMPLE
    .\Parse-DotNet6Report.ps1 -CsvPath ".\report.csv" -OutputPath ".\DotNet6Apps.csv" -Detailed

.NOTES
    FileName: Parse-DotNet6Report.ps1
    Author: Anders Ahl
    Created: 2026-06-02
    Updated: 2026-06-02
    Version: 1.0.0
    Requires: PowerShell 5.1 or higher
    Use Case: Post-processing of MWPW_PRD_DotNet6Detect Intune Remediation output
...
param(
    ...
    [Parameter(Mandatory = $false, HelpMessage = "Exclude known .NET SDK/runtime tooling paths from results.")]
    [switch]$PostFilter,
    ...
    try {
        ...
        if (-not $outputColumn) {
            foreach ($col in $columnNames) {
                $sampleValue = ($csvData | Where-Object { $_.$col -match '\.NET 6' } | Select-Object -First 1).$col
                if ($sampleValue) {
                    $outputColumn = $col
                    break
                }
        ...
        foreach ($row in $csvData) {
            ...
            # Only process non-compliant devices
            if ($detectionOutput -notmatch 'NON-COMPLIANT') { continue }

            $NonCompliantCount++
            $deviceName = if ($deviceColumn) { $row.$deviceColumn } else { "Unknown" }

            # Split pipe-delimited output segments
        ...
        # Apply post-filter to remove SDK/runtime tooling paths
        if ($PostFilter) {
            ...
            foreach ($app in $ParsedApps) {
                if ($app.AppPath -notlike '*\dotnet\sdk\*' -and $app.AppPath -notlike '*\dotnet\shared\*') {
                    [void]$filteredApps.Add($app)
                }
            }
            $ParsedApps = $filteredApps
            $removedCount = $preFilterCount - $ParsedApps.Count
            Write-Host "Post-filter removed $($removedCount) SDK/runtime entries (dotnet\sdk, dotnet\shared)"
        }

        if ($ParsedApps.Count -eq 0) {
            Write-Host "No .NET 6.x dependent applications found in the report."
            return
        }
        ...
        # Display summary
        Write-Host "=== .NET 6.x Dependent Applications ===" -ForegroundColor Cyan
        Write-Host ""
...
End {
    $Timer.Stop()
    Write-Host ""
    Write-Host "--- Parsing complete in $($Timer.Elapsed.TotalSeconds.ToString('N1')) second(s) ---"
    if ($ErrorCount -gt 0) {
